...
class StringTR{
  ...
  showAdress(e){
    ...
    // let parent = document.querySelector('.adress');
    let parent = e.target;
...
function render (){
  ...
  node.innerHTML = `
    ...
    <th data-type="number" onclick="sortTable(event)" style="cursor: pointer" onmouseover="this.style.background = 'yellow'" onmouseout="this.style.background = ''">Кол-во employers</th>
...
function renderEmp (company){
  ...
  node2.innerHTML = `
    ...
    <button onclick="findTable()">Find</button>
  `;
...
//   let rowsArray = [].slice.call(tbody.rows);
//   let result = rowsArray.find(function(e){return e.cells[0].innerHTML==item});
//   while (tbody.firstChild){
//     tbody.removeChild(tbody.firstChild);
//   };
